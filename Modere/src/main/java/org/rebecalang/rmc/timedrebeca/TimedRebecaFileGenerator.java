...
public class TimedRebecaFileGenerator extends CoreRebecaFileGenerator {
	...
	@Override
	protected void addSymmetryContext(VelocityContext context) {
		org.rebecalang.rmc.SymmetryAnalyzer.Result symmResult =
				org.rebecalang.rmc.SymmetryAnalyzer.analyze(rebecaModel);
		context.put("symmTypeIds", symmResult.typeIds);
		context.put("symmKRTable", symmResult.krTable);
		context.put("symmKRSize", symmResult.krSize);
	}
	...
	protected void createTimedModelChecker() throws IOException {
		...
		org.rebecalang.rmc.PORSafetyAnalyzer.Result porResult =
				org.rebecalang.rmc.PORSafetyAnalyzer.analyze(rebecaModel);
		context.put("safeActionsTable", porResult.safeActionsTable);
		context.put("maxMsgIdPlusOne", porResult.maxMsgId + 1);
		...
		context.put("symmKRTable", symmResult.krTable);
		context.put("symmKRSize", symmResult.krSize);
		context.put("symmMaxKnown", symmResult.maxKnown);
