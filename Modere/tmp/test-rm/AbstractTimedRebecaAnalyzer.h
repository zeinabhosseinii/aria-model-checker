...
protected:
	...
	static const byte symmTypeIds[REBEC_COUNT];
	static const int symmKRSize[REBEC_COUNT];
	static const byte symmKRTable[REBEC_COUNT][MAX_KNOWN];
	void sort(TimedRebecStateIndexType* sys);
	bool check(int i, int j, byte* perm, TimedRebecStateIndexType* sys);
	bool isAssigned(int r, byte* perm);
	#endif // SYMM
