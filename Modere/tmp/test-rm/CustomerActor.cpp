...
void CustomerActor::marshalVariablesToArrayWithPerm(byte* array, byte* perm) {
    int _mi = 0;
        memcpy(&array[_mi], &_ref_id, (1 * BYTE_SIZE));
        _mi += (1 * BYTE_SIZE);
array[_mi] = 0;
array[_mi] |= (_ref_sent ? 0xFF : 0x00) & AbstractActor::coeff[0];
_mi++;
}

void CustomerActor::marshalParamsWithPerm(byte* output, byte* params, byte msgType, byte* perm) {
