...
void AgentActor::marshalVariablesToArrayWithPerm(byte* array, byte* perm) {
    int _mi = 0;
        memcpy(&array[_mi], &_ref_shared, (1 * INT_SIZE));
        _mi += (1 * INT_SIZE);
}

void AgentActor::marshalParamsWithPerm(byte* output, byte* params, byte msgType, byte* perm) {
